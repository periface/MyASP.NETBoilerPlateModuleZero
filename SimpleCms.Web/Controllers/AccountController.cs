...
using SimpleCms.Web.Controllers.Results;
...
{
    public class AccountController : SimpleCmsControllerBase
    {
        ...
        {
            CheckModelState();
            var tenancyFoundName = await GetTenancyNameByUrl();
            if (!string.IsNullOrEmpty(loginModel.TenancyName))
            {
                tenancyFoundName = loginModel.TenancyName;
            }
        ...
        /// <summary>
        /// Retrieves the tenancy name by the provided url if empty returns "" (host)
        /// </summary>
        /// <returns></returns>
        private async Task<string> GetTenancyNameByUrl()
        {
            if (!_multiTenancyConfig.IsEnabled) return string.Empty;
            ...
            var lookOut = await _tenantManager.FindByTenancyNameAsync(tenancyName);
            return lookOut != null ? lookOut.TenancyName : "";
        }
        ...
        public virtual async Task<ActionResult> Register(RegisterViewModel model)
        {
            try
            {
                ...
                var tenant = await GetActiveTenantAsync(tenancyFoundName);
